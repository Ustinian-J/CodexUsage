...
Remote monitoring is opt-in and stores only a persistent enabled flag, validated SSH config aliases, and per-host recovery timestamps. New and upgraded installations keep the flag off until the user explicitly enables it. While enabled, CodexS keeps at most one long-lived system OpenSSH channel per host and automatically reconnects with delays from 10 seconds to a five-minute cap; disabling it stops CodexS-owned channels and cancels future retries. CodexS launches the platform OpenSSH client with batch authentication, strict host-key checking, connection timeout, and keepalive limits. On macOS it resolves `ControlPath` locally with `ssh -G`, accepts only an existing Unix socket owned by the current user, verifies the master with `ssh -O check`, and reuses it through a randomized link inside a mode-0700 temporary directory. The original socket is never replaced or removed, and stopping CodexS closes only its multiplexed channel. If no verified master exists, a temporary local config disables `ControlMaster`, `ControlPersist`, and `ControlPath` before including the user's and system SSH aliases, so the same isolation also reaches implicit `ProxyJump` helpers; temporary files contain no credentials and are removed when monitoring stops. The host value is passed as a separate validated argument, never interpolated into a local shell. A fixed Python 3 parser runs in memory on the remote host, opens Codex state/session files read-only, emits only allowlisted task metadata, and does not create or modify remote files. OpenSSH may use keys or an agent configured by the user, but CodexS never opens, copies, logs, or persists those credentials.
