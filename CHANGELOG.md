...
- Add opt-in local quota threshold notifications after the core build is verified.
- Add an Apple Silicon build job when a matching clean runner is selected and verified.
...
- Added a least-privilege Intel GitHub Actions build: official actions pinned to full verified commits, `contents: read`, no secrets, self-tests, DMG verification, and SHA-256 artifact output.
