# CodexS Design System

本文档定义 CodexS 的长期视觉与交互规范。它不是某个版本或某次改版的复盘，而是后续所有界面迭代都应遵守的设计边界。

## 设计立场

CodexS 是一个 macOS 菜单栏和桌面工作型工具。它应该安静、清晰、可信，优先服务高频扫视和快速判断，而不是制造展示感。
...
- 任务状态在紧凑状态栏中采用“单一主状态 + 独立注意角标”：红色播放符号为运行、绿色勾为空闲、灰色横线为不可用，黄色菱形为未读完成。形状与图标必须让状态不依赖颜色辨识；红黄绿完整灯组只用于有文字标签的弹窗卡片。
- 默认丰富模式用上下两条固定色进度条同时展示 5h 与 7d；这与主流用量菜单栏工具的双条仪表一致。任务注意角标叠加于主状态，不占用第三套并列状态空间。
