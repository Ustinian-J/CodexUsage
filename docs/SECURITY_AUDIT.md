...
## 2026-08-24 orphan-task liveness addendum

Codex rollout logs do not always contain a matching `task_complete` or `turn_aborted` after an interrupted process. The macOS monitor therefore uses the fixed system binary `/usr/sbin/lsof` to test only the exact normalized paths of locally observed rollout files with unmatched starts. The probe does not use a shell, is capped at 128 candidates and 2 MiB per output stream, drains output while running, has a five-second process timeout with forced termination fallback, and treats every launch, parse, timeout, or permission failure as unknown rather than idle. A local task is removed only after its rollout has been unchanged for at least 30 seconds and two successful samples show that no process holds any known copy open. Remote tasks are excluded from this local reconciliation, and cleanup creates no completion result or notification.
