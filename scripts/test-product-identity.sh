...
/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' Resources/Info.plist \
  | grep -qx '9'
grep -q '当前版本为 `0.4.2`' README.md
grep -q 'The current version is `0.4.2`' README.en.md
...
grep -q '<Version>0.4.2</Version>' windows/src/CodexS.Windows/CodexS.Windows.csproj
grep -q 'CodexS-0.4.2-windows-x64.exe' .github/workflows/windows.yml
